  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 glass animate-fade-in transition-all">
      <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between relative">
        <a href={import.meta.env.BASE_URL} className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600 tracking-tighter">
          Shiyam
        </a>

        {/* Desktop Menu */}
        <nav className="hidden md:flex items-center gap-8 font-medium text-sm text-gray-600">
          <a href={`${import.meta.env.BASE_URL}#work`} className="hover:text-gray-900 transition-colors">Work</a>
          <a href={`${import.meta.env.BASE_URL}#about`} className="hover:text-gray-900 transition-colors">About</a>
          <a href={`${import.meta.env.BASE_URL}#contact`} className="hover:text-gray-900 transition-colors">Contact</a>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-20 left-0 right-0 bg-white/95 backdrop-blur-md shadow-lg border-t border-gray-100 p-6 flex flex-col gap-6 animate-fade-in-up">
          <a href={`${import.meta.env.BASE_URL}#work`} onClick={() => setIsOpen(false)} className="text-lg font-medium text-gray-800 hover:text-gray-900">Work</a>
          <a href={`${import.meta.env.BASE_URL}#about`} onClick={() => setIsOpen(false)} className="text-lg font-medium text-gray-800 hover:text-gray-900">About</a>
          <a href={`${import.meta.env.BASE_URL}#contact`} onClick={() => setIsOpen(false)} className="text-lg font-medium text-gray-800 hover:text-gray-900">Contact</a>
